...
public class TitleCommand extends VanillaCommand{
	...
	public TitleCommand(String name){
		super(name, "%bozkurt.command.title.description", "%bozkurt.command.title.usage");
		this.setPermission("bozkurt.command.title");
	...
	public boolean execute(CommandSender sender, String commandLabel, String[] args){
		...
		if(args.length == 2){
			switch(args[1].toLowerCase()){
				case "clear":
					player.clearTitle();
					sender.sendMessage(new TranslationContainer("bozkurt.command.title.clear", player.getName()));
					break;
				case "reset":
					player.resetTitleSettings();
					sender.sendMessage(new TranslationContainer("bozkurt.command.title.reset", player.getName()));
					break;
				default:
					sender.sendMessage(new TranslationContainer("commands.generic.usage", this.usageMessage));
					return false;
			...
			switch(args[1].toLowerCase()){
				case "title":
					...
					sender.sendMessage(new TranslationContainer("bozkurt.command.title.title", new String[]{TextFormat.clean(args[2]), player.getName()}));
					break;
                /*case "subtitle":
                    player.setSubtitle(args[2]);
                    sender.sendMessage(new TranslationContainer("bozkurt.command.title.subtitle", new String[]{TextFormat.clean(args[2]), player.getName()}));
                    break;
                case "actionbar":
                    player.sendActionBarTitle(args[2]);
                    sender.sendMessage(new TranslationContainer("bozkurt.command.title.actionbar", new String[]{TextFormat.clean(args[2]), player.getName()}));
                    break;*/
				default:
					sender.sendMessage(new TranslationContainer("commands.generic.usage", this.usageMessage));
					return false;
			...
			if(args[1].toLowerCase().equals("times")){
				try{
					...
					sender.sendMessage(new TranslationContainer("bozkurt.command.title.times.success", new String[]{
							args[2], args[3], args[4], player.getName()}));
				}catch(NumberFormatException exception){
					sender.sendMessage(new TranslationContainer(TextFormat.RED + "%bozkurt.command.title.times.fail"));
				}
			}else{
				sender.sendMessage(new TranslationContainer("commands.generic.usage", this.usageMessage));
				return false;
