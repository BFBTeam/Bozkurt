	...
	public ZippedResourcePack(File file){
		if(!file.exists()){
			throw new IllegalArgumentException(Server.getInstance().getLanguage()
					.translateString("bozkurt.resources.zip.not-found", file.getName()));
		}
		...
		try(ZipFile zip = new ZipFile(file)){
			...
			if(entry == null){
				throw new IllegalArgumentException(Server.getInstance().getLanguage()
						.translateString("bozkurt.resources.zip.no-manifest"));
			}else{
		...
		if(!this.verifyManifest()){
			throw new IllegalArgumentException(Server.getInstance().getLanguage()
					.translateString("bozkurt.resources.zip.invalid-manifest"));
		}
	}
