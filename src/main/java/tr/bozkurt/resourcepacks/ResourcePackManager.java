	...
	public ResourcePackManager(File path){
		if(!path.exists()){
			...
			throw new IllegalArgumentException(Server.getInstance().getLanguage()
					.translateString("bozkurt.resources.invalid-path", path.getName()));
		}
		...
		for(File pack : path.listFiles()){
			try{
					...
					switch(Files.getFileExtension(pack.getName())){
						...
						default:
							Server.getInstance().getLogger().warning(Server.getInstance().getLanguage()
									.translateString("bozkurt.resources.unknown-format", pack.getName()));
							break;
					}
				}
				...
				Server.getInstance().getLogger().warning(Server.getInstance().getLanguage()
						.translateString("bozkurt.resources.fail", pack.getName(), e.getMessage()));
			}
		}
		...
		Server.getInstance().getLogger().info(Server.getInstance().getLanguage()
				.translateString("bozkurt.resources.success", String.valueOf(this.resourcePacks.length)));
	}
