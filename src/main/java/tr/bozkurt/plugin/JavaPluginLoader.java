...
import tr.bozkurt.event.plugin.PluginDisableEvent;
import tr.bozkurt.event.plugin.PluginEnableEvent;
...
public class JavaPluginLoader implements PluginLoader{
	...
	public Plugin loadPlugin(File file) throws Exception{
		...
		if(description != null){
			this.server.getLogger().info(this.server.getLanguage().translateString("bozkurt.plugin.load", description.getFullName()));
	...
	public PluginDescription getPluginDescription(File file){
		try(JarFile jar = new JarFile(file)){
			JarEntry entry = jar.getJarEntry("bozkurt.yml");
			if(entry == null){
	...
	public void enablePlugin(Plugin plugin){
		if(plugin instanceof PluginBase && !plugin.isEnabled()){
			this.server.getLogger().info(this.server.getLanguage().translateString("bozkurt.plugin.enable", plugin.getDescription().getFullName()));
	...
	public void disablePlugin(Plugin plugin){
		if(plugin instanceof PluginBase && plugin.isEnabled()){
			this.server.getLogger().info(this.server.getLanguage().translateString("bozkurt.plugin.disable", plugin.getDescription().getFullName()));
