...
import tr.bozkurt.command.PluginCommand;
import tr.bozkurt.command.SimpleCommandMap;
import tr.bozkurt.event.*;
...
public class PluginManager{
					...
					try{
						...
						if(description != null){
							...
							if(plugins.containsKey(name) || this.getPlugin(name) != null){
								this.server.getLogger().error(this.server.getLanguage().translateString("bozkurt.plugin.duplicateError", name));
								continue;
							}
							...
							for(String version : description.getCompatibleAPIs()){
								...
								if(!Pattern.matches("[0-9]\\.[0-9]\\.[0-9]", version)){
									this.server.getLogger().error(this.server.getLanguage().translateString("bozkurt.plugin.loadError", new String[]{name, "Wrong API format"}));
									continue;
								}
							...
							if(!compatible){
								this.server.getLogger().error(this.server.getLanguage().translateString("bozkurt.plugin.loadError", new String[]{name, "%bozkurt.plugin.incompatibleAPI"}));
							}
						...
						this.server.getLogger().error(this.server.getLanguage().translateString("bozkurt.plugin" +
				...
				for(String name : new ArrayList<>(plugins.keySet())){
					...
					if(dependencies.containsKey(name)){
						for(String dependency : new ArrayList<>(dependencies.get(name))){
							if(loadedPlugins.containsKey(dependency) || this.getPlugin(dependency) != null){
								...
								this.server.getLogger().critical(this.server.getLanguage().translateString("bozkurt" +
										".plugin.loadError", new String[]{name, "%bozkurt.plugin.unknownDependency"}));
								break;
							}
						}
					...
					if(!dependencies.containsKey(name) && !softDependencies.containsKey(name)){
						...
						if(plugin != null){
							...
							this.server.getLogger().critical(this.server.getLanguage().translateString("bozkurt.plugin.genericLoadError", name));
						}
					}
				}
				...
				if(missingDependency){
					...
					if(missingDependency){
						for(String name : plugins.keySet()){
							this.server.getLogger().critical(this.server.getLanguage().translateString("bozkurt.plugin.loadError", new String[]{name, "%bozkurt.plugin.circularDependency"}));
						}
						plugins.clear();
					}
				}
	...
	protected List<PluginCommand> parseYamlCommands(Plugin plugin){
		...
		for(Map.Entry entry : plugin.getDescription().getCommands().entrySet()){
			...
			if(key.contains(":")){
				this.server.getLogger().critical(this.server.getLanguage().translateString("bozkurt.plugin.commandError", new String[]{key, plugin.getDescription().getFullName()}));
				continue;
			}
				...
				if(((Map) data).containsKey("aliases")){
					...
					if(aliases instanceof List){
						...
						for(String alias : (List<String>) aliases){
							if(alias.contains(":")){
								this.server.getLogger().critical(this.server.getLanguage().translateString("bozkurt.plugin.aliasError", new String[]{alias, plugin.getDescription().getFullName()}));
								continue;
							}
							aliasList.add(alias);
						}
		...
		try{
			for(RegisteredListener registration : getEventListeners(event.getClass()).getRegisteredListeners()){
				...
				try{
					...
					this.server.getLogger().critical(this.server.getLanguage().translateString("bozkurt.plugin.eventError", event.getEventName(), registration.getPlugin().getDescription().getFullName(), e.getMessage(), registration.getListener().getClass().getName()));
			...
			for(Class<?> clazz = eventClass; Event.class.isAssignableFrom(clazz); clazz = clazz.getSuperclass()){
				...
				if(clazz.getAnnotation(Deprecated.class) != null){
					if(Boolean.valueOf(String.valueOf(this.server.getConfig("settings.deprecated-verbpse", true)))){
						this.server.getLogger().warning(this.server.getLanguage().translateString("bozkurt.plugin.deprecatedEvent", plugin.getName(), clazz.getName(), listener.getClass().getName() + "." + method.getName() + "()"));
					}
					break;
				}
			}
