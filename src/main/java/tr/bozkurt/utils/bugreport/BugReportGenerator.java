package tr.bozkurt.utils.bugreport;
...
public class BugReportGenerator extends Thread{
	...
	public void run(){
		...
		try{
			Server.getInstance().getLogger().info("[BugReport] " + baseLang.translateString("bozkurt.bugreport.create"));
			String path = generate();
			Server.getInstance().getLogger().info("[BugReport] " + baseLang.translateString("bozkurt.bugreport.archive", path));
		}catch(Exception e){
			StringWriter stringWriter = new StringWriter();
			e.printStackTrace(new PrintWriter(stringWriter));
			Server.getInstance().getLogger().info("[BugReport] " + baseLang.translateString("bozkurt.bugreport.error", stringWriter.toString()));
		}
	}

	private String generate() throws IOException{
		File reports = new File(Bozkurt.DATA_PATH, "logs/bug_reports");
		...
		if(stackTrace.length > 0){
			pluginError = !throwable.getStackTrace()[0].getClassName().startsWith("tr.bozkurt");
		}
		...
		content = content.replace("${BOZKURT_VERSION}", Bozkurt.VERSION);
