 ...
 * @author null(javadoc) @ Bozkurt Project
 * @see tr.bozkurt.event.Listener
...
public @interface EventHandler{
	 ...
	 *
	 * <p>Bozkurt调用处理器时会按照优先级从低到高的顺序调用，这样保证了高优先级的监听器能覆盖低优先级监听器做出的处理。
	 * 调用的先后顺序如下：<br> </p>
	 * When Bozkurt calls all handlers, ones with lower priority is called earlier,
	 * that make handlers with higher priority can replace the decisions made by lower ones.
	 * The order that Bozkurt call handlers is from the first to the last as:
	 * <ol>
