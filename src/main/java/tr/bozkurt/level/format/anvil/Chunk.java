package tr.bozkurt.level.format.anvil;
...
import tr.bozkurt.level.format.anvil.palette.BiomePalette;
import tr.bozkurt.level.format.generic.BaseChunk;
...
import tr.bozkurt.utils.BlockUpdateEntry;
...
public class Chunk extends BaseChunk{
	...
	public Chunk(LevelProvider level, CompoundTag nbt){
		...
		if(nbt == null){
			...
			this.sections = new tr.bozkurt.level.format.ChunkSection[16];
			...
			for(CompoundTag entryNBT : updateEntries.getAll()){
				...
				try{
					...
					if(tag instanceof StringTag){
						...
						Class<? extends Block> clazz = (Class<? extends Block>) Class.forName("tr.bozkurt.block." + name);
...
//            chunk.lightPopulated = false;
	...
	public byte[] toFastBinary(){
		...
		for(tr.bozkurt.level.format.ChunkSection section : this.getSections()){
	...
	public int getBlockSkyLight(int x, int y, int z){
		tr.bozkurt.level.format.ChunkSection section = this.sections[y >> 4];
		if(section instanceof tr.bozkurt.level.format.anvil.ChunkSection){
			tr.bozkurt.level.format.anvil.ChunkSection anvilSection = (tr.bozkurt.level.format.anvil.ChunkSection) section;
	...
	public boolean compress(){
		...
		for(tr.bozkurt.level.format.ChunkSection section : getSections()){
