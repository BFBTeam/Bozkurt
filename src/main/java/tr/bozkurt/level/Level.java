...
import tr.bozkurt.block.BlockRedstoneDiode;
import tr.bozkurt.blockentity.BlockEntity;
import tr.bozkurt.blockentity.BlockEntityChest;
import tr.bozkurt.blockentity.BlockEntityShulkerBox;
import tr.bozkurt.collection.PrimitiveList;
import tr.bozkurt.entity.Entity;
import tr.bozkurt.entity.item.EntityItem;
import tr.bozkurt.entity.item.EntityXPOrb;
import tr.bozkurt.entity.projectile.EntityArrow;
import tr.bozkurt.entity.weather.EntityLightning;
import tr.bozkurt.event.block.BlockBreakEvent;
import tr.bozkurt.event.block.BlockPlaceEvent;
import tr.bozkurt.event.block.BlockUpdateEvent;
import tr.bozkurt.event.level.*;
import tr.bozkurt.event.player.PlayerInteractEvent;
import tr.bozkurt.event.player.PlayerInteractEvent.Action;
import tr.bozkurt.event.weather.LightningStrikeEvent;
import tr.bozkurt.inventory.InventoryHolder;
import tr.bozkurt.item.Item;
import tr.bozkurt.item.ItemBlock;
import tr.bozkurt.item.enchantment.Enchantment;
import tr.bozkurt.level.format.Chunk;
import tr.bozkurt.level.format.ChunkSection;
...
import tr.bozkurt.level.format.generic.BaseLevelProvider;
import tr.bozkurt.level.format.generic.EmptyChunkSection;
...
import tr.bozkurt.level.generator.PopChunkManager;
import tr.bozkurt.level.generator.task.GenerationTask;
import tr.bozkurt.level.generator.task.LightPopulationTask;
import tr.bozkurt.level.generator.task.PopulationTask;
import tr.bozkurt.level.particle.DestroyBlockParticle;
import tr.bozkurt.level.particle.Particle;
import tr.bozkurt.math.*;
import tr.bozkurt.math.BlockFace.Plane;
import tr.bozkurt.metadata.BlockMetadataStore;
import tr.bozkurt.metadata.MetadataValue;
import tr.bozkurt.metadata.Metadatable;
...
import tr.bozkurt.scheduler.BlockUpdateScheduler;
...
//    private final List<BlockUpdateEntry> nextTickUpdates = Lists.newArrayList();
	...
	private IterableThreadLocal<Generator> generators = new IterableThreadLocal<Generator>(){
		...
		public Generator init(){
			try{
				...
				BozkurtRandom rand = new BozkurtRandom(getSeed());
	...
	public Level(Server server, String name, String path, Class<? extends LevelProvider> provider){
		...
		if(convert){
			this.server.getLogger().info(this.server.getLanguage().translateString("bozkurt.level.updating",
		...
		this.server.getLogger().info(this.server.getLanguage().translateString("bozkurt.level.preparing",
				TextFormat.GREEN + this.provider.getName() + TextFormat.WHITE));
	...
	public boolean unload(boolean force){
		...
		this.server.getLogger().info(this.server.getLanguage().translateString("bozkurt.level.unloading",
...
//        }
	...
	public Entity[] getCollidingEntities(AxisAlignedBB bb, Entity entity){
		...
		if(entity == null || entity.canCollide()){
			int minX = BozkurtMath.floorDouble((bb.getMinX() - 2) / 16);
			int maxX = BozkurtMath.ceilDouble((bb.getMaxX() + 2) / 16);
			int minZ = BozkurtMath.floorDouble((bb.getMinZ() - 2) / 16);
			int maxZ = BozkurtMath.ceilDouble((bb.getMaxZ() + 2) / 16);
	...
	public Entity[] getNearbyEntities(AxisAlignedBB bb, Entity entity, boolean loadChunks){
		...
		int minX = BozkurtMath.floorDouble((bb.getMinX() - 2) * 0.0625);
		int maxX = BozkurtMath.ceilDouble((bb.getMaxX() + 2) * 0.0625);
		int minZ = BozkurtMath.floorDouble((bb.getMinZ() - 2) * 0.0625);
		int maxZ = BozkurtMath.ceilDouble((bb.getMaxZ() + 2) * 0.0625);
	...
	public synchronized boolean unloadChunk(int x, int z, boolean safe, boolean trySave){
		...
		try{
			...
			logger.error(this.server.getLanguage().translateString("bozkurt.level.chunkUnloadError", e.toString()));
	...
	public Position getSafeSpawn(Vector3 spawn){
		...
		if(spawn != null){
			...
			int z = (int) v.z & 0x0f;
			if(chunk != null){
				int y = (int) BozkurtMath.clamp(v.y, 0, 254);
	...
	public boolean isAreaLoaded(AxisAlignedBB bb){
		...
		int minX = BozkurtMath.floorDouble(bb.getMinX()) >> 4;
		int minZ = BozkurtMath.floorDouble(bb.getMinZ()) >> 4;
		int maxX = BozkurtMath.floorDouble(bb.getMaxX()) >> 4;
		int maxZ = BozkurtMath.floorDouble(bb.getMaxZ()) >> 4;
