...
import tr.bozkurt.utils.ChunkException;
...
public abstract class Entity extends Location implements Metadatable{
	...
	protected boolean checkObstruction(double x, double y, double z){
		...
		int i = BozkurtMath.floorDouble(x);
		int j = BozkurtMath.floorDouble(y);
		int k = BozkurtMath.floorDouble(z);
	...
	public BlockFace getHorizontalFacing(){
		return BlockFace.fromHorizontalIndex(BozkurtMath.floorDouble((this.yaw * 4.0F / 360.0F) + 0.5D) & 3);
	}
	...
	protected boolean updateRidden(){
		if(this.linkedEntity != null){
			...
			if(this.linkedEntity != null){
				...
				float var5 = 10.0F;
				var1 = BozkurtMath.clamp(var1, -var5, var5);
				var3 = BozkurtMath.clamp(var3, -var5, var5);
				this.YawDelta -= var1;
	...
	public void applyEntityCollision(Entity entity){
		if(entity.riding != this && entity.linkedEntity != this){
			...
			double dz = BozkurtMath.getDirection(dx, dy);
	...
	public boolean isInsideOfWater(){
		...
		Block block = this.level.getBlock(this.temporalVector.setComponents(BozkurtMath.floorDouble(this.x), BozkurtMath.floorDouble(y), BozkurtMath.floorDouble(this.z)));
	...
	public boolean isInsideOfSolid(){
		...
		Block block = this.level.getBlock(
				this.temporalVector.setComponents(
						BozkurtMath.floorDouble(this.x),
						BozkurtMath.floorDouble(y),
						BozkurtMath.floorDouble(this.z))
		);
	...
	public List<Block> getBlocksAround(){
		if(this.blocksAround == null){
			int minX = BozkurtMath.floorDouble(this.boundingBox.getMinX());
			int minY = BozkurtMath.floorDouble(this.boundingBox.getMinY());
			int minZ = BozkurtMath.floorDouble(this.boundingBox.getMinZ());
			int maxX = BozkurtMath.ceilDouble(this.boundingBox.getMaxX());
			int maxY = BozkurtMath.ceilDouble(this.boundingBox.getMaxY());
			int maxZ = BozkurtMath.ceilDouble(this.boundingBox.getMaxZ());
	 ...
	 * Used for {@link tr.bozkurt.entity.passive.EntityBat}s only.
	 *
